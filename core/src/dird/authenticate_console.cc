...
ConsoleAuthenticator::ConsoleAuthenticator(UaContext *ua, const std::string &console_name)
  : auth_success_(false)
  , console_name_(console_name)
  , ua_(ua)
{
  optional_console_resource_ = (ConsoleResource *)my_config->GetResWithName(R_CONSOLE, console_name_.c_str());
}
...
OptionResult ConsoleAuthenticator::DoDefaultAuthentication()
{
  ...
  if (console_name_ != default_console_name) {
    return OptionResult::Skipped;
  }

  auth_success_ = ua_->UA_sock->AuthenticateInboundConnection(NULL, "Console",
                                                              default_console_name.c_str(),
                                                              me->password_,
                                                              me);
  ua_->cons = nullptr;
...
void ConsoleAuthenticator::DoNamedAuthentication()
{
  ...
  auth_success_ = ua_->UA_sock->AuthenticateInboundConnection(NULL, "Console",
                                                              console_name_.c_str(),
                                                              optional_console_resource_->password_,
                                                              optional_console_resource_);
  if (auth_success_) {
...
{
  public:
    ConsoleAuthenticatorBefore_18_2(UaContext *ua, const std::string &console_name)
...
{
   if (DoDefaultAuthentication() == OptionResult::Skipped) { return OptionResult::Skipped; }
...
OptionResult ConsoleAuthenticatorBefore_18_2::AuthenticatePamUser()
{
  ...
  if (optional_console_resource_->use_pam_authentication_) {
    ...
    auth_success_ = false; /* console before 18_2 cannot do pam authentication */
    return OptionResult::Completed;
  }
  return OptionResult::Skipped;
}
...
bool ConsoleAuthenticatorFrom_18_2::SendResponseMessage(uint32_t response_id, bool send_version_info)
{
  std::string message;
...
bool ConsoleAuthenticatorFrom_18_2::SendInfoMessage()
{
  ...
  auth_success_ = ua_->UA_sock->FormatAndSendResponseMessage(kMessageIdInfoMessage, message);
  return true;
}
...
void ConsoleAuthenticatorFrom_18_2::AuthenticateNamedConsole()
{
  ...
  if (optional_console_resource_ && optional_console_resource_->use_pam_authentication_) {
...
#else /* HAVE_PAM */
{
  ...
  if (!optional_console_resource_->use_pam_authentication_) { return OptionResult::Skipped; }

  uint32_t response_id;
  BStringList message_arguments;

  if (!ua_->UA_sock->ReceiveAndEvaluateResponseMessage(response_id, message_arguments)) {
  ...
  if (!PamAuthenticateUser(ua_->UA_sock, pam_username, pam_password, authenticated_username)) {
    ua_->cons = nullptr;
    auth_success_ = false;
  } else {
...
static bool GetConsoleNameAndVersion(BareosSocket *ua_sock, std::string &name_out, BareosVersionNumber &version_out)
{
  std::string name;
  std::string r_code_str_unused;
  BareosVersionNumber version = BareosVersionNumber::kUndefined;

  if (GetNameAndResourceTypeAndVersionFromHello(ua_sock->msg, name, r_code_str_unused, version)) {
    name_out = name;
    version_out = version;
    return true;
  } else {
...
static ConsoleAuthenticator* CreateConsoleAuthenticator(UaContext *ua)
{
  ...
  ua->UA_sock->connected_daemon_version_ = version; /* this is redundant if using cleartext handshake */
...
bool AuthenticateConsole(UaContext *ua)
{
  ...
  std::unique_ptr<ConsoleAuthenticator> console_authenticator(CreateConsoleAuthenticator(ua));
  if (!console_authenticator) { return false; }

  if (console_authenticator->AuthenticateDefaultConsole() == OptionResult::Completed) {
    ...
    if (console_authenticator->AuthenticatePamUser() == OptionResult::Completed) {
      if (!console_authenticator->auth_success_) {
        LogErrorMessage(console_authenticator->console_name_, ua);
        return false;
      }
